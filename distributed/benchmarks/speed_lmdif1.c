...
void do_test(int ctx, int nprob, int n, int m, double factor)
{
	...
	int info = plmdif1(fcn, &nprob, m, n, x, fvec, tol, ctx);	// find solution
...
int main()
{
	...
	double start = MPI_Wtime();
	do_test(ctx, 1, 1000, 1000, 1);
	do_test(ctx, 1, 1000, 10000, 1);
	do_test(ctx, 1, 1000, 100000, 1);
	do_test(ctx, 16, 1000, 1000, 1);
	do_test(ctx, 16, 3000, 3000, 1);
	do_test(ctx, 12, 3, 1000000, 1);
